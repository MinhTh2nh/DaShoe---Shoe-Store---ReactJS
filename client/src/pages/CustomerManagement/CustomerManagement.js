...
function ProductManagement() {
  ...
  const handleClickOpen = (customer) => {
    if (customer.customer_id) {
      ...
      setcustomerBirth(customer.customer_dob);
  ...
  const fetchCustomers = () => {
    axios
      .get(`${PUBLIC_API_URL}/api/customers`)
  ...
  const handleAddProduct = (customer) => {
    axios
      .post(`${PUBLIC_API_URL}/api/customers`, customer)
      .then(() => {
        fetchCustomers();
  ...
  const handleUpdateProduct = (customer) => {
    axios
      .put(`${PUBLIC_API_URL}/api/customers/${customer.customer_id}`, customer)
      .then(() => {
        fetchCustomers();
        alert('The action has been successfully executed.');
      })
    ...
    <section className={cx('wrapper')}>
      <div className={cx('function-site')}>
        ...
        <Dialog open={open} onClose={handleClose}>
          <DialogTitle>{customer.customer_id ? 'Edit Customer' : 'Add Customer'}</DialogTitle>
          ...
          <DialogActions>
            ...
            >
              {customer.customer_id ? 'Update Entry' : 'Submit'}
            </Button>
          </DialogActions>
        </Dialog>
      ...
      <div className={cx('table-site')}>
        <div className={cx('table')}>
          ...
          {filtered.map((cus) => {
            return (
              <div key={cus.customer_id} className={cx('table-grid', 'item-grid')}>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_id}</p>
                </div>
                <div className={cx('item-site1')}>
                  <p className={cx('item-content')}>{cus.customer_name}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_phone}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_email}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_dob.toLocaleString('en-US')}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_address}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_city}</p>
                </div>
                <div className={cx('item-site')}>
                  <p className={cx('item-content')}>{cus.customer_gender}</p>
                </div>
                <div className={cx('item-site')}>
